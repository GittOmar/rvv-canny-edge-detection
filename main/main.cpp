...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < 100; i++) {
        Image b = gaussian_blur(src);
        if (i < 99) image_free(b);
        else {
            Image blurred = b;

            // Stage 2: Sobel
            clock_gettime(CLOCK_MONOTONIC, &t1);
            GradientImage grad = sobel(blurred);
    ...
    double gaussian_ms = get_ms(t0, t_end) / 100.0;
